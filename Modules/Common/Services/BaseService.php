...
class BaseService
{
    ...
    public function apiSuccess(string $message = '', array $data = array(), int $status = StatusData::Ok)
    {
        ...
        return response()->json([
            ...
            'message' => $message,
            'data' => $data
        ], CodeData::OK)->setEncodingOptions(JSON_UNESCAPED_UNICODE);
    }
    ...
    public function commonAddAll($model, array $data = [], string $successMessage = MessageData::ADD_API_SUCCESS, string $errorMessage = MessageData::ADD_API_ERROR)
    {
    ...
    public function commonCreateLastId($model,array $data = [],string $successMessage = MessageData::ADD_API_SUCCESS,string $errorMessage = MessageData::ADD_API_ERROR){
        $data['created_at'] = date('Y-m-d H:i:s');
        $data['updated_at'] = date('Y-m-d H:i:s');
        if ($model->insert($data)){
            $lastId = DB::getPdo()->lastInsertId();
            return $this->apiSuccess($successMessage, ['lastId' => $lastId]);
        }
        $this->apiError($errorMessage);
    }

    public function getAdvertiserInfo($model, $id)
    {
        return $model->where([['advertiser_id', '=', $id], ['state', '=', 1]])->first();
    }
}
