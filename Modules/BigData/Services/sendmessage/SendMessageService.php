...
class SendMessageService extends BaseApiService
{
...
//        $array = array('13538116492','18664335586');
     ...
     */
    public function send_dd($info,$res,$keys){
        $jqr = $this->robot();
        $phone = '@'.$this->userphone();
        $media_name = "平台:  ".$this->getMediaName($info->media_id).PHP_EOL;
        $user = '账户:  '.$info->name.PHP_EOL;
        $sign = '';
        if($res->fie_name == 'returnoninvestment'){ $sign = "(低于预警值)";}//投资回报率
        if($res->fie_name == 'collectioncost'){ $sign = "(高于预警值)";}//投资回报率
        $remark = $res->name.":  [".$info->$keys."]   ".$sign.PHP_EOL."请及时处理".$phone;
        $message=$media_name.$user.$remark;
        ...
        if($body['errmsg'] == 'ok'){
            ...
            DB::table('brand_send_datas')->insert([
                ...
                'fie_name' => $res->fie_name,
                'values' => $info->$keys,
                'keyword' => md5($info->memberid.$info->$keys),
                'keys' => $info->key_sign,
                'created_at' => Carbon::now()
            ]);
            DB::table('brand_account_reports')->where('id',$info->id)->update(['send_type' => 1,'updated_at' => Carbon::now()]);
        }
    }

    /**
     * @Notes:轮询数据库,查询预警的数据 (老版本数据查询)
     * @Author: 1900
    ...
    {
        //Redis::set("advertise",'23a5f78f8fd3738a2e1c105461c6250e');  //account_plan_reports
        //$advertise = Redis::get('advertise');
        $advertise = '';
        if(!$advertise){
            $brand_id = DB::table("brand_account_reports")->max('id');
            $advertise = DB::table('brand_account_reports')->where('id',$brand_id)->value('key_sign');
        }
        $reportmodel = DB::table('brand_account_reports')->where('key_sign',$advertise)->get()->count();

        foreach($reportmodel as $k=>$v){
            if(isset($v->collectioncost)){
                $v->collectioncost = round($v->collectioncost/100,2);
            }
            //var_dump($v);die;
            $this->alert_Setting($v);
        }
    }
     ...
     * @Interface alert_Setting
     * @param $key
     */
    public function alert_Setting($key)
    {
        //投资回报率
        $returnoninvestment = DB::table('brand_alert_settings')->where('fie_name','returnoninvestment')->first();
        $keyword = md5($key->memberid.$key->returnoninvestment);
        $keyword_info = DB::table("brand_send_datas")->where('keyword',$keyword)->where('type',1)->first();
        $keys = $returnoninvestment->fie_name;
        if ($returnoninvestment && ($key->$keys < $returnoninvestment->values) && !$keyword_info){
            $this->compare_Early($key,$returnoninvestment,$keys);
        }
        //消耗
        $collectioncost =  DB::table('brand_alert_settings')->where('fie_name','collectioncost')->first();
        $keyword1 = md5($key->memberid.$key->collectioncost);
        $collectioncost_info = DB::table("brand_send_datas")->where('keyword',$keyword1)->where('type',1)->first();
        $keys = $collectioncost->fie_name;
        //var_dump($keys);die;
        if ($collectioncost && ($key->$keys > $collectioncost->values) && !$collectioncost_info){
            $this->compare_Early($key,$collectioncost,$keys);
        }
    }
     ...
     * @Notes:预警数据 预警规则
     * @Author: 1900
     * @Date: 2022/8/16 15:31
     * @Interface compare_Early
     * @param $info
     * @param $res
     * @param $keys
     */
    ...
    public function newEarly()
    {
        ...
        $reportmodel = $reportmodel
            ->select('media_id','name','id','key_sign','memberid','returnoninvestment','collectioncost')
            ->groupBy('id')->get();

       //var_dump($reportmodel);die;
        foreach($reportmodel as $k=>$v){
    ...
    public function compareData($info,$retu,$coll)
    {
        if ($retu->values > $info->returnoninvestment) {
            //投资回报率
            $keyword = md5($info->memberid.$info->returnoninvestment);
            $keyword_info = DB::table("brand_send_datas")->where('keyword', $keyword)->where('type', 1)->first();
            $keys = $retu->fie_name;
        ...
        if ($coll->values > $info->collectioncost) {
            //消耗
            $keyword1 = md5($info->memberid.$info->collectioncost);
            $collectioncost_info = DB::table("brand_send_datas")->where('keyword', $keyword1)->where('type', 1)->first();
            $keys = $coll->fie_name;
